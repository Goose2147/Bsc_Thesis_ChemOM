...
from lammps_prism import prism 
...
class OPLSff:
    ...
    def read(self, fileobj, comments='#'):
        ...
        # CHANGE THE NUMBER(second arg) DEPENDING ON THE LENGTH OF YOUR BONDS NAMES(Ca1-Cb1 -> 7), ANGLES NAMES etc.
        read_block('one', 3, 3)
        read_block('bonds', 7, 2)
        read_block('angles', 11, 2)
        read_block('dihedrals', 14, 3)
        read_block('cutoffs', 7, 1)
...
""")
    ...
    def write_lammps_atoms(self, atoms, connectivities):
        ...
        if atoms.has('molid'):
            ...
            molid = [1] * len(atoms)
        for i, r in enumerate(map(p.pos_to_lammps_fold_str,
                                  atoms.get_positions())):
    ...
    def get_bonds(self, atoms):
        ...
        print(self.data['cutoffs'])
        cutoffs = CutoffList(self.data['cutoffs'])
        print(cutoffs.value('C12', 'H10'))
        ...
        for i, atom in enumerate(atoms):
            ...
            for j, offset in zip(indices, offsets):
                ...
                jname = types[tags[j]]
                #print(f'{iname} around it {jname}')
                cut = cutoffs.value(iname, jname)
                #print(f'{iname}-{jname} , cut: {cut}')
                if cut is None:
                    if self.warnings > 1:
